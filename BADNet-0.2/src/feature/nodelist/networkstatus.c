...
client_register()
{
  ...
  PyRun_SimpleString("sys.path.append('/home/BADNet/src/SC/')"); 
...
download_MSRI_with_keys_from_blockchain()
{
  ...
  FILE *fp = fopen("/usr/local/BADNet/lib/MSRI", "w");
                           ...
                           const char *source_dir)
{
  ...
  if (is_client) {
    const char *filename = "/usr/local/BADNet/lib/cached-microdesc-consensus"; 
    const char *name = "/usr/local/BADNet/lib/MSRI";

    if (strcmp(fname, filename) == 0) {
      if (client_register()){
        log_info(LD_DIR, "Wait until the next epcho!");
      } else {
        if (init_reload == 0) {
          RIA_update();
          init_reload = 1;
