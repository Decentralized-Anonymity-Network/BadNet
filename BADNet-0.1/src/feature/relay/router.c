...
router_upload_descriptor_to_blockchain(routerinfo_t *ri)
{
  ...
  // ******
  // (1)Generate symmetric key and (2)SRI encryption will be added here! 
  // ******

  if (python_is_initialized == false) {
    Py_Initialize();
    if (!Py_IsInitialized()) {
      log_info(LD_DIR, "Py_Initialize failed!");
      return ;
    }
  ...
  if (flag == 0) {
    ...
    pResVal = PyObject_CallObject(pFunc, pArgs);
    log_info(LD_DIR, "Relay %s registers in the network.", ri->nickname);
  }
  
  pFunc = PyDict_GetItemString(pDict, "relay_get_clientlist_length");
  pResVal = PyObject_CallObject(pFunc, pArgs);
  long clientlist_length = PyLong_AsLong(pResVal);
  char *publicKey[clientlist_length];
  /*
  //Download clients public keys.
  for (int i=0; i<clientlist_length; i++) {
    pFunc = PyDict_GetItemString(pDict, "relay_download_clients_public_keys");
    pArgs = PyTuple_New(1);
    PyTuple_SetItem(pArgs, 0, Py_BuildValue("i", i));
    pResVal = PyObject_CallObject(pFunc, pArgs);
    publicKey[i] = PyUnicode_FromString(pResVal);
  }
  */

  // ******
  // Public key encryption process will be added here! 
  // ******
  char *keys = NULL;
  ...
  PyTuple_SetItem(pArgs, 1, Py_BuildValue("s", keys));
  pResVal = PyObject_CallObject(pFunc, pArgs);
  log_info(LD_DIR, "Successfully upload SRI to blockchain!");
...
router_rebuild_descriptor(int force, int flag)
{
  ...
  // ************
  if (flag == 1){
    router_upload_descriptor_to_blockchain(ri);
  }
