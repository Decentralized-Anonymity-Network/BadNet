...
client_register()
{
  if (python_is_initialized == false) {
    Py_Initialize();
    if (!Py_IsInitialized()) {
      log_info(LD_FS, "Py_Initialize failed");
    }
  ...
  PyRun_SimpleString("sys.path.append('./../../SC/')"); 

  pModule = PyImport_ImportModule("Client");
  if (!pModule) {
    log_debug(LD_DIR, "Load python file failed!");
    return ;
  }  
  pDict = PyModule_GetDict(pModule);
  if (!pDict) {
    log_debug(LD_DIR, "Can't find dict in python file!");
    return ;
  }
  ...
  if (flag == 0) {
    ...
    pResVal = PyObject_CallObject(pFunc, pArgs);
    log_info(LD_DIR, "Client registration in the network.");
...
download_MSRI_with_keys_from_blockchain()
{
  ...
  pFunc = PyDict_GetItemString(pDict, "client_get_RIA_length");
  pResVal = PyObject_CallObject(pFunc, pArgs);
  long RIA_length = PyLong_AsLong(pResVal);

  char *MSRI[RIA_length], *encryptedKey[RIA_length];
  log_info(LD_DIR, "Launching MSRI with keys download.");
  for (int i=0; i<RIA_length; i++) {
    ...
    /*
    pFunc = PyDict_GetItemString(pDict, "client_download_encrypted_keys");
    pArgs = PyTuple_New(1);
    PyTuple_SetItem(pArgs, 0, Py_BuildValue("i", i));
    pResVal = PyObject_CallObject(pFunc, pArgs);
    encryptedKey[i] = PyByteArray_AS_STRING(pResVal);
    */
  }

  // ******
  // The decryption process will be added here!
  // (1) private key + encryptedKey[i] -> Key[i]
  // (2) Key[i] + MSRI[i] -> Plaintext_MSRI[i]
  // ******

  FILE *fp = fopen("/usr/local/BADNet/lib/tor/MSRI", "w");
  for (int j=0; j<RIA_length; j++) {
    fprintf(fp, "%s", MSRI[j]);
  }
  fclose(fp);
                           ...
                           const char *source_dir)
{
  ...
  if (is_client) {
    const char *filename = "/usr/local/BADNet/lib/tor/cached-microdesc-consensus"; 
    const char *name = "/usr/local/BADNet/lib/tor/MSRI";

    if (strcmp(fname, filename) == 0) {
      client_register();

      if (access(name, 0) == -1) {
        /* If MSRI does not exist locally. */
        download_MSRI_with_keys_from_blockchain();
      } else {
        FILE *fp = fopen(name, "r");
        long modify_time, now;
        if (fp != NULL) {
          int fd = fileno(fp);
          struct stat buf;
          fstat(fd, &buf);
