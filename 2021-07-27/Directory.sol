...
contract Directory{
    ...
    function relay_upload_SRI_and_keys(bytes info, bytes[] keys) public{
        ...
        for(uint i=0; i<relay[num].clientlist.length; i++){
            clientKey[relay[num].clientlist[i]].encryptedKey[num] = keys[i];
        }
    ...
    function createRandom(address addr, uint8 j) private view returns (uint16){
        return uint16(uint(keccak256(abi.encodePacked(block.timestamp, block.difficulty, now + j, addr))) % (relayIndex-1) + 1);
    }
